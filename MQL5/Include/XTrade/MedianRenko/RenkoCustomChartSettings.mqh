...
//
      ...
      int                           barSizeInTicks = (int)GET(BrickSize);               // Renko body size (in ticks)
      ENUM_CUSTOM_BAR_TYPE          PredefinedSetting= (ENUM_CUSTOM_BAR_TYPE)GET(RenkoType);         // Renko mode preset
      int                           InpOpen=50;                         // Open offset % (0 to ..)
      int                           InpRevShadow=50;                    // Reversal Open offset % (0 to ..)
      int                           InpRev=150;                         // Reversal bar size % (0 to ..)
      ENUM_BOOL                     showWicks = GET(Wicks);                   // Show wicks
      ...
      input ENUM_CUSTOM_BAR_TYPE    PredefinedSetting = cbtPointO;         // Renko mode preset
      ...
      int                           barSizeInTicks = (int)GET(BrickSize);               // Range bar size (in ticks)
      ENUM_CUSTOM_BAR_TYPE          PredefinedSetting=(ENUM_CUSTOM_BAR_TYPE)GET(RenkoType);         // Renko mode preset
      int                           InpOpen=50;                         // Open offset % (0 to ..)
      int                           InpRevShadow=50;                    // Reversal Open offset % (0 to ..)
      int                           InpRev=150;                         // Reversal bar size % (0 to ..)
      
      ENUM_BOOL                     showWicks = (ENUM_BOOL)GET(Wicks);                   // Show wicks
      ENUM_BOOL                     atrEnabled = false;                 // Enable ATR based bar size calculation
      ENUM_TIMEFRAMES               atrTimeFrame = PERIOD_D1;           // Use ATR period
      int                           atrPeriod = 14;                     // ATR period
      int                           atrPercentage = 10;                 // Use percentage of ATR
...
#include <XTrade/MedianRenko/CustomChartSettingsBase.mqh>
