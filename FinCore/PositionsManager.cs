...
namespace FinCore
{
    public class PositionsManager : ITerminalEvents
    {
        ...
        public PositionsManager()
        {
            ...
            foreach (var t in (List<object>) xtrade.GetObjects(EntitiesEnum.Terminal))
            {
                var term = (Terminal) t;
                if (term.Retired)
                    continue;
            ...
            lock (lockObject)
            {
                ...
                foreach (var notcontains in posMagic)
                    if (!positionsToAdd.ContainsKey(notcontains.Ticket))
                    {
                        ...
                        notcontains.Profit = (double) xtrade.ConvertToUSD(new decimal(notcontains.Profit),
                            terminals[AccountNumber].Currency);
                        notcontains.Value = (double) xtrade.ConvertToUSD(new decimal(notcontains.calculateValue()),
